...
class Creates {
    ...
    async business(data) {
        if (this._SESSION.user.sessionId) {
    ...
    async store(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            let store = await stores.build(data);
            store.businessId = await this._SESSION.user.businessId;
            store.sessionId = await this._SESSION.user.sessionId;
    ...
    async syllabus(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            syllabus.businessId = await this._SESSION.user.businessId;
            syllabus.sessionId = await this._SESSION.user.sessionId;
    ...
    async deduction(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            data.businessId = await this._SESSION.user.businessId;
            data.sessionId = await this._SESSION.user.sessionId;
    ...
    async deduction_charts(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            deduction_charts.businessId = await this._SESSION.user.businessId;
            deduction_charts.sessionId = await this._SESSION.user.sessionId;
            ...
            if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
                ...
                for (let i = 0; i < data.qty.length; i++) {
                    let dt = {
                        ...
                        businessId: await this._SESSION.user.businessId,
                        sessionId: await this._SESSION.user.sessionId,
    ...
    async bookIssue(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            book.businessId = await this._SESSION.user.businessId;
            book.sessionId = await this._SESSION.user.sessionId;
    ...
    async bus(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            bus.businessId = await this._SESSION.user.businessId;
            bus.sessionId = await this._SESSION.user.sessionId;
    ...
    async attendance(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            attendance.businessId = await this._SESSION.user.businessId;
            attendance.sessionId = await this._SESSION.user.sessionId;
    ...
    async mark(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            mark.businessId = await this._SESSION.user.businessId;
            mark.sessionId = await this._SESSION.user.sessionId;
    ...
    async teacher_permissions(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            try {
                ...
                tperm.businessId = await this._SESSION.user.businessId;
                tperm.sessionId = await this._SESSION.user.sessionId;
    ...
    async currency(data) {
        if (this._SESSION.user.sessionId, this._SESSION.user.userId) {
    ...
    async country(data) {
        if (this._SESSION.user.sessionId, this._SESSION.user.userId) {
            let country = await countries.build(data);
    ...
    async classroom(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            classroom.businessId = await this._SESSION.user.businessId;
            classroom.sessionId = await this._SESSION.user.sessionId;
    ...
    async classes(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            classe.businessId = await this._SESSION.user.businessId;
            classe.sessionId = await this._SESSION.user.sessionId;
    ...
    async section(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            section.businessId = await this._SESSION.user.businessId;
            section.sessionId = await this._SESSION.user.sessionId;
    ...
    async subject(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            subject.businessId = await this._SESSION.user.businessId;
            subject.sessionId = await this._SESSION.user.sessionId;
    ...
    async user(data) {
        if (typeof data.phone == typeof []) {
            ...
            nUser.sessionId = await this._SESSION.user.sessionId;
            await nUser.save();
    ...
    async personel(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            try {
                ...
                personel.businessId = await this._SESSION.user.businessId;
                personel.sessionId = await this._SESSION.user.sessionId;
                let pp = await personel.save();
                data.personelId = pp.id;

                let user_role = await user_roles.build(data);
                user_role.businessId = await this._SESSION.user.businessId;
                user_role.roleId = 4;
                user_role.save();
    ...
    async department(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            department.businessId = await this._SESSION.user.businessId;
            department.sessionId = await this._SESSION.user.sessionId;
    ...
    async teacher(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            try {
                ...
                teacher.businessId = await this._SESSION.user.businessId;
                teacher.sessionId = await this._SESSION.user.sessionId;
                await teacher.save()
    ...
    async student(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            try {
                ...
                student.businessId = await this._SESSION.user.businessId;
                student.sessionId = await this._SESSION.user.sessionId;
                await student.save()
        ...
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            try {
                ...
                for (var i = 0; i < data.name.length; i++) {
                    ...
                    let customer = { name: data.name[i], code: code, address: data.address[i], phone: data.phone[i], email: data.email[i], tin: data.tin[i], personelId: parseInt(data.personelId[i]), storeId: parseInt(data.storeId[i]), sessionId: this._SESSION.user.sessionId, businessId: (this._SESSION.user.businessId) ? this._SESSION.user.businessId : 1 };
                    console.log('customer>>>>>>', customer);
                    let c = await customers.create(customer);
                    c = JSON.parse(JSON.stringify(await c));
                    console.log(c);

                    let enr = { personelId: parseInt(data.personelId[i]), storeId: parseInt(data.storeId[i]), sessionId: this._SESSION.user.sessionId, businessId: (this._SESSION.user.businessId) ? this._SESSION.user.businessId : 1, customerId: parseInt(c.id) };
    ...
    async bus_route(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            bus_route.businessId = await this._SESSION.user.businessId;
            bus_route.sessionId = await this._SESSION.user.sessionId;
    ...
    async drop_off(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            drop_off.businessId = await this._SESSION.user.businessId;
            drop_off.sessionId = await this._SESSION.user.sessionId;
    ...
    async students_route(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            students_route.businessId = await this._SESSION.user.businessId;
            students_route.sessionId = await this._SESSION.user.sessionId;
    ...
    async salary(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            salary.businessId = await this._SESSION.user.businessId;
            salary.sessionId = await this._SESSION.user.sessionId;
    ...
    async routine(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            routine.businessId = await this._SESSION.user.businessId;
            routine.sessionId = await this._SESSION.user.sessionId;
    ...
    async expenses_category(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            data['businessId'] = this._SESSION.user.businessId;
            data['sessionId'] = this._SESSION.user.sessionId;
    ...
    async grade(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            grade.businessId = await this._SESSION.user.businessId;
            grade.sessionId = await this._SESSION.user.sessionId;
    ...
    async invoice(data) {
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            invoice.businessId = await this._SESSION.user.businessId;
            invoice.sessionId = await this._SESSION.user.sessionId;
        ...
        if (this._SESSION.user.businessId, this._SESSION.user.sessionId) {
            ...
            for (let i = 0; i < data.itemId.length; i++) {
                let tdata = {
                    ...
                    businessId: this._SESSION.user.businessId,
                }
