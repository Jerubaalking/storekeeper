...
router.get('/', isLoggedIn, async (req, res) => {
    ...
    let business = JSON.parse(JSON.stringify(await (await control.findBy()).business({})));
    let business_count = JSON.parse(JSON.stringify(await (await businesses.count())));
...
router.post('/enter', isLoggedIn, async (req, res) => {
    ...
    res.status(200).json({ status: true, linking: '/client' });
});
